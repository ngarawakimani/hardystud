...
<div id="tribe-events-daynum-<?php echo esc_attr($day['daynum']) ?>">
	...
	<?php if ( $day['total_events'] > 0 && tribe_events_is_view_enabled( 'day' ) ) : ?>
		<a href="<?php echo esc_url( tribe_get_day_link( $day['date'] ) ); ?>"><?php echo esc_attr($day['daynum']) ?></a>
	<?php else : ?>
		<?php echo esc_attr($day['daynum']) ?>
	<?php endif; ?>
...
<?php if ( $day['view_more'] ) : ?>
	<div class="tribe-events-viewmore">
		<?php
			...
			$view_all_label = sprintf( _n( 'View 1 %1$s', 'View All %2$s %3$s', $day['total_events'], 'happyrider' ), $events_label_singular, $day['total_events'], $events_label_plural );

		?>
		<a href="<?php echo esc_url( $day['view_more'] ); ?>"><?php echo esc_attr($view_all_label) ?> &raquo;</a>
	</div>
<?php
endif;
