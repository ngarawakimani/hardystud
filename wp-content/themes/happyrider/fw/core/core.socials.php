<?php
/**
 * HappyRider Framework: social networks
 *
 * @package	happyrider
...
// Theme init
if (!function_exists('happyrider_socials_theme_setup')) {
	add_action( 'happyrider_action_before_init_theme', 'happyrider_socials_theme_setup' );
	function happyrider_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'happyrider_facebook_og_tags', 5 );
		}
...
$HAPPYRIDER_GLOBALS['share_links'] = array(
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=happyrider',
...
if (!function_exists('happyrider_add_share_link')) {
	function happyrider_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		global $HAPPYRIDER_GLOBALS;
		$HAPPYRIDER_GLOBALS['share_links'] = array_merge( $HAPPYRIDER_GLOBALS['share_links'], $soc );
	}
}

// Return (and show) share social links
if (!function_exists('happyrider_show_share_links')) {
	function happyrider_show_share_links($args) {
		if ( happyrider_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'style' => happyrider_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons',	// style for show icons: icons|images|bg
			'type' => 'block',					// share block type: list|block|drop
			'popup' => true,					// open share url in new window or in popup window
			'counters' => happyrider_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => happyrider_get_custom_option('show_share'),				// share block direction
			'caption' => happyrider_get_custom_option('share_caption'),				// share block caption
			'share' => happyrider_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				$icon = $args['style']=='icons' || happyrider_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : happyrider_get_socials_url(basename($soc['icon']));
				if ($args['style'] == 'icons') {
				...
				$url = empty($soc['url']) && !empty($HAPPYRIDER_GLOBALS['share_links'][$sn]) ? $HAPPYRIDER_GLOBALS['share_links'][$sn] : $soc['url'];
				...
				$output .= '<div class="sc_socials_item">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(happyrider_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('happyrider_prepare_socials')) {
	function happyrider_prepare_socials($list, $style='') {
		if (empty($style)) $style = happyrider_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || happyrider_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : happyrider_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('happyrider_get_twitter_data')) {
	function happyrider_get_twitter_data($cfg) {
		$data = get_transient("twitter_data_".($cfg['mode']));
		if (!$data) {
			require_once( happyrider_get_file_dir('lib/tmhOAuth/tmhOAuth.php') );
			...
			$code = $tmhOAuth->user_request(array(
				'url' => $tmhOAuth->url(happyrider_get_twitter_mode_url($cfg['mode']))
			));
			if ($code == 200) {
				$data = json_decode($tmhOAuth->response['response'], true);
				if (isset($data['status'])) {
					$code = $tmhOAuth->user_request(array(
						'url' => $tmhOAuth->url(happyrider_get_twitter_mode_url($cfg['oembed'])),
						'params' => array(
		...
		} else if (!is_array($data) && happyrider_substr($data, 0, 2)=='a:') {
...
if (!function_exists('happyrider_get_twitter_mode_url')) {
	function happyrider_get_twitter_mode_url($mode) {
		$url = '/1.1/statuses/';
...
if (!function_exists('happyrider_prepare_twitter_text')) {
	function happyrider_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('happyrider_get_twitter_followers')) {
	function happyrider_get_twitter_followers($cfg) {
		$data = happyrider_get_twitter_data($cfg);
...
if (!function_exists('happyrider_get_facebook_likes')) {
	function happyrider_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(happyrider_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (happyrider_strpos($headers[0], '200')) {
			$json = happyrider_fgc($url);
...
function happyrider_facebook_og_tags() {
	global $post;
	if ( !is_singular() || happyrider_get_global('blog_streampage')) return;
...
if (!function_exists('happyrider_get_feedburner_counter')) {
	function happyrider_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(happyrider_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (happyrider_strpos($headers[0], '200')) {
			$xml = happyrider_fgc($url);
