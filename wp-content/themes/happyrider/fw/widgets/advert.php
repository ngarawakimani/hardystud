 ...
 */
add_action( 'widgets_init', 'happyrider_widget_advert_load' );

/**
 * Register our widget.
 */
function happyrider_widget_advert_load() {
	register_widget( 'happyrider_widget_advert' );
}

/**
 * Twitter Widget class.
 */
class happyrider_widget_advert extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_advert', 'description' => __('Advertisement block', 'happyrider') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'happyrider_widget_advert' );

		/* Create the widget. */
		parent::__construct( 'happyrider_widget_advert', __('HappyRider - Advertisement block', 'happyrider'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_advert_inner">
			...
			if ($advert_code!='') {
				echo force_balance_tags(happyrider_substitute_all($advert_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => __('Advertisement block', 'happyrider') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_image' )); ?>"><?php _e('Image source URL:<br />(leave empty if you paste advert code)', 'happyrider'); ?></label>
            ...
            <?php
			echo trim(happyrider_show_custom_field($this->get_field_id( 'advert_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'advert_image' )), null));
			if ($advert_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_link' )); ?>"><?php _e('Image link URL:<br />(leave empty if you paste advert code)', 'happyrider'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_code' )); ?>"><?php _e('or paste Advert Widget HTML Code:', 'happyrider'); ?></label>
