 ...
 */
add_action( 'widgets_init', 'happyrider_widget_socials_load' );

/**
 * Register our widget.
 */
function happyrider_widget_socials_load() {
	register_widget( 'happyrider_widget_socials' );
}

/**
 * flickr Widget class.
 */
class happyrider_widget_socials extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => __('Show site logo and social links', 'happyrider') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'happyrider_widget_socials' );

		/* Create the widget. */
		parent::__construct( 'happyrider_widget_socials', __('HappyRider - Show logo and social links', 'happyrider'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? happyrider_do_shortcode($instance['text']) : '';
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					...
					if ($logo_image=='')	$logo_image = $HAPPYRIDER_GLOBALS['logo'];
					if ($logo_text=='')		$logo_text = $HAPPYRIDER_GLOBALS['logo_text'];
					if ($logo_slogan=='')	$logo_slogan = $HAPPYRIDER_GLOBALS['logo_slogan'];
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'logo_image'=>'', 'logo_text'=>'', 'show_logo' => '1', 'show_icons' => '1', 'text'=>'', 'description' => __('Show logo and social icons', 'happyrider') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php _e('Description:', 'happyrider'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_image' )); ?>"><?php _e('Logo image:<br />(if empty - use logo from Theme Options)', 'happyrider'); ?></label>
            ...
            <?php
			echo trim(happyrider_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_text' )); ?>"><?php _e('Logo text:', 'happyrider'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php _e('Show logo:', 'happyrider'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php _e('Show', 'happyrider'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_0"><?php _e('Hide', 'happyrider'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php _e('Show social icons:', 'happyrider'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php _e('Show', 'happyrider'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_0"><?php _e('Hide', 'happyrider'); ?></label>
		</p>
