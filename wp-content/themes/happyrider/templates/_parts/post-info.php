			...
			<<?php echo esc_attr($info_parts['tag']); ?> class="post_info">
				...
				if ($info_parts['author']) {
					?>
					<span class="post_info_item post_info_posted_by<?php echo ($info_parts['snippets'] ? ' vcard' : ''); ?>"<?php echo ($info_parts['snippets'] ? ' itemprop="author"' : ''); ?>><?php _e('by', 'happyrider'); ?> <a href="<?php echo esc_url($post_data['post_author_url']); ?>" class="post_info_author"><?php echo ($post_data['post_author']); ?></a></span>
				<?php 
				}
				if ($info_parts['counters']) {
					?>
					<span class="post_info_item post_info_counters"><?php require(happyrider_get_file_dir('templates/_parts/counters.php')); ?></span>
					<?php
				}
				...
				if (is_single() && !happyrider_get_global('blog_streampage') && ($post_data['post_edit_enable'] || $post_data['post_delete_enable'])) {
					?>
					<span class="frontend_editor_buttons">
						<?php if ($post_data['post_edit_enable']) { ?>
						<span class="post_info_item post_info_button post_info_button_edit"><a id="frontend_editor_icon_edit" class="icon-pencil" title="<?php _e('Edit post', 'happyrider'); ?>" href="#"><?php _e('Edit', 'happyrider'); ?></a></span>
						<?php } ?>
						<?php if ($post_data['post_delete_enable']) { ?>
						<span class="post_info_item post_info_button post_info_button_delete"><a id="frontend_editor_icon_delete" class="icon-trash" title="<?php _e('Delete post', 'happyrider'); ?>" href="#"><?php _e('Delete', 'happyrider'); ?></a></span>
						<?php } ?>
					</span>
					<?php
				}
