...
if ( !function_exists( 'happyrider_template_services_3_theme_setup' ) ) {
	add_action( 'happyrider_action_before_init_theme', 'happyrider_template_services_3_theme_setup', 1 );
	function happyrider_template_services_3_theme_setup() {
		happyrider_add_template(array(
			'layout' => 'services-3',
			'template' => 'services-3',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => __('Services /Style 3/', 'happyrider'),
...
// Template output
if ( !function_exists( 'happyrider_template_services_3_output' ) ) {
	function happyrider_template_services_3_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					echo happyrider_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
				} else {
					?>
