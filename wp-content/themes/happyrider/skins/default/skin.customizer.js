...
function happyrider_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = HAPPYRIDER_GLOBALS['co_add_params']!=undefined ? HAPPYRIDER_GLOBALS['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#happyrider-customizer-styles-'+option).length > 0 ? jQuery('#happyrider-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="happyrider-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
