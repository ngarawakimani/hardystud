...
class wp_booking_calendar_public_lists {	
	...
	public function getSlotsPerDayList($year,$month,$day,$calendar_id,$bookingSettingObj) {
		...
		if($year."-".$month."-".$day == date('Y-m-d')) {
			...
			$slotsQry = $wpdb->get_results("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots WHERE slot_active=1 AND slot_date = '".$year."-".$month."-".$day."' AND calendar_id=".$calendar_id."  AND REPLACE(slot_time_from,':','') >= DATE_FORMAT(NOW(),'%H%i%s') ORDER BY slot_time_from");
		...
		for($i=0;$i<count($slotsQry);$i++) {
			...
			if($bookingSettingObj->getSlotsUnlimited() == 0 && $bookingSettingObj->getShowBookedSlots() == 0) {
				$reservationQry=$wpdb->query($wpdb->prepare("SELECT reservation_id FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE slot_id=%d AND reservation_cancelled = %d",$slotsQry[$i]->slot_id,0));
				if($reservationQry==0) {
					$arraySlots[$slotsQry[$i]->slot_id] = Array();
					$arraySlots[$slotsQry[$i]->slot_id]["slot_time_from"] = $slotsQry[$i]->slot_time_from;
