...
if($_GET["calendar_id"] > 0) {
	...
	foreach($arrayMonth as $daynum => $daydata) {
		if($i == 0) {
			...
			for($j=$startDay;$j<$daydata["dayofweek"];$j++) {
				?>
				<div class="booking_day_container booking_day_container_custom booking_day_grey booking_font_14"><a style="background-color:<?php echo $bookingSettingObj->getDayGreyBg(); ?>" class="booking_font_14"></a></div>
				<?php
			}
		}
		...
		$daynumstyle = 'style="color:'.$bookingSettingObj->getDayWhiteLine1Color().'"';
		$newstyle2 = 'style="color:'.$bookingSettingObj->getDayWhiteLine2Color().';background-color:'.$bookingSettingObj->getDayWhiteLine2Bg().'"';
		$over=1;
        ...
        if($daydata["yearnum"].$daydata["monthnum"].$daydata["daynum"] < $bookfromdate || $numslots == -1 || $daydata["yearnum"].$daydata["monthnum"].$daydata["daynum"] > $booktodate) {
            ...
            $daynumstyle = 'style="color:'.$bookingSettingObj->getDayWhiteLine1DisabledColor().'"';
            $newstyle2 = 'style="color:'.$bookingSettingObj->getDayWhiteLine2DisabledColor().';background-color:'.$bookingSettingObj->getDayWhiteLine2DisabledBg().'"';
            $over=0;
        }
        ...
        if($numslots == 0 && $daydata["yearnum"].$daydata["monthnum"].$daydata["daynum"] >= date('Ymd')) {
            ...
            $daynumstyle = 'style="color:'.$bookingSettingObj->getDayRedLine1Color().'"';
            $newstyle2 = 'style="color:'.$bookingSettingObj->getDayRedLine2Color().';background-color:'.$bookingSettingObj->getDayRedLine2Bg().'"';
		...
		if($daydata["dayofweek"] == $lastWeekDay) {
			...
			?>
			<div class="booking_day_container booking_day_container_custom <?php echo $background; ?> booking_font_14" style="margin-right: 0px;"><a class="booking_font_14" style="cursor:pointer;background-color:<?php echo $background_color; ?>" year="<?php echo $_GET["year"]; ?>" month="<?php echo $_GET["month"]; ?>" day="<?php echo $daynum; ?>" popup="<?php echo $slots_popup_enabled; ?>" over="<?php echo $over; ?>">
				<div class="booking_day_number booking_font_14" <?php echo $daynumstyle; ?>><?php echo $daynum; ?></div>
				...
				<div class="booking_day_slots booking_font_14" <?php echo $newstyle2; ?>>
					...
					if($numslots>0 && $daydata["yearnum"].$daydata["monthnum"].$daydata["daynum"] >= $bookfromdate && $daydata["yearnum"].$daydata["monthnum"].$daydata["daynum"] <= $booktodate) {
                        ...
                        echo __( 'SOLDOUT', 'wp-booking-calendar' );
                    }
					?>
				</div>
		...
		/* all other days*/
			...
			?>
			<div class="booking_day_container booking_day_container_custom <?php echo $background; ?> booking_font_14"><a class="booking_font_14" style="cursor:pointer;background-color:<?php echo $background_color; ?>" year="<?php echo $_GET["year"]; ?>" month="<?php echo $_GET["month"]; ?>" day="<?php echo $daynum; ?>" popup="<?php echo $slots_popup_enabled; ?>" over="<?php echo $over; ?>">
				<div class="booking_day_number booking_font_14" <?php echo $daynumstyle; ?>><?php echo $daynum; ?></div>
	...
	for($j=$lastWeekDay;$j>$lastDay;$j--) {
		?>
		<div class="booking_day_container booking_day_container_custom booking_day_grey booking_font_14"><a class="booking_font_14" style="background-color:<?php echo $bookingSettingObj->getDayGreyBg(); ?>"></a></div>
		<?php
	}
	?>
	<script>
		var $wbc = jQuery;
		$wbc(function() {
			$wbc('#booking_month_nav_prev').html("<a href=\"javascript:getBookingPreviousMonth(<?php echo $bookingCalendarObj->getCalendarId(); ?>,'<?php echo $_GET["publickey"]; ?>',<?php echo $bookingSettingObj->getCalendarMonthLimitPast(); ?>);\" class=\"booking_month_nav_button booking_month_navigation_button_custom fa fa-angle-left\"></a>");
			$wbc('#booking_month_nav_next').html("<a href=\"javascript:getBookingNextMonth(<?php echo $bookingCalendarObj->getCalendarId(); ?>,'<?php echo $_GET["publickey"]; ?>',<?php echo $bookingSettingObj->getCalendarMonthLimitFuture(); ?>);\" class=\"booking_month_nav_button booking_month_navigation_button_custom fa fa-angle-right\"></a>");
		});
	</script>
<?php
} else {
	...
	foreach($arrayMonth as $daynum => $daydata) {
		...
		if($daydata["dayofweek"] == $lastWeekDay) {
			...
			?>
			<div class="booking_day_container booking_day_container_custom <?php echo $background; ?> booking_font_14"><a class="booking_font_14" style="cursor:pointer;background-color:<?php echo $background_color; ?>" year="<?php echo $_GET["year"]; ?>" month="<?php echo $_GET["month"]; ?>" day="<?php echo $daynum; ?>" over="<?php echo $over; ?>">
				<div class="booking_day_number booking_font_14" <?php echo $newstyle; ?>><?php echo $daynum; ?></div>
				<div class="booking_day_slots booking_font_14" <?php echo $newstyle; ?>>
