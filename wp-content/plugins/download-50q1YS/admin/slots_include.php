...
<a href="javascript:showPage('new_slot');" class="booking_manage_slot_box_title booking_border_1 booking_border_solid booking_border_ccc"><?php echo __( 'Create Time Slots', 'wp-booking-calendar' ); ?></a>
...
<script>
	...
	function filterSlots() {
		if($wbc("#search_date option:selected").val()==1) {
			...
			if(Trim($wbc('#first_date').val()) != '') {
				...
				$wbc.ajax({
				  url: '<?php echo BOOKING_ADMIN_URL;?>ajax/filterSlots.php?search_date=1&date_from='+$wbc('#first_date').val()+"&time_from="+$wbc('#time_from_hour').val()+":"+$wbc('#time_from_minute').val()+":"+$wbc('#time_from_ampm').val()+"&time_to="+$wbc('#time_to_hour').val()+":"+$wbc('#time_to_minute').val()+":"+$wbc('#time_to_ampm').val()+"&calendar_id=<?php echo $_GET["calendar_id"]; ?>&pag=1",
			...
			$wbc('#result_search').html('<img src="<?php echo BOOKING_ADMIN_URL;?>images/loading.gif">');
			$wbc.ajax({
			  url: '<?php echo BOOKING_ADMIN_URL;?>ajax/filterSlots.php?search_date=2&date_from='+$wbc('#first_date').val()+"&weekday="+$wbc('#slot_week_day').val()+"&date_to="+$wbc('#second_date').val()+"&time_from="+$wbc('#time_from_hour').val()+":"+$wbc('#time_from_minute').val()+":"+$wbc('#time_from_ampm').val()+"&time_to="+$wbc('#time_to_hour').val()+":"+$wbc('#time_to_minute').val()+":"+$wbc('#time_to_ampm').val()+"&calendar_id=<?php echo $_GET["calendar_id"]; ?>&pag=1",
			  success: function(data) {
				 $wbc('#table').hide().html(data).fadeIn(2000);
				 $wbc('#result_search').html('');
				 goToByScroll("results");
	...
	function showFilters(value) {
		if(value==1) {
			...
			$wbc('#label_from').html("<?php echo esc_js(__( 'Day', 'wp-booking-calendar' )); ?>");
			$wbc('#date_to_filter').slideUp();
			$wbc('#weekdays_filter').slideUp();
		} else if(value==2) {
			$wbc('#filters').slideDown();
			$wbc('#label_from').html("<?php echo esc_js(__( 'From', 'wp-booking-calendar' )); ?>");
	...
	function orderby(column,type) {
		...
		$wbc.ajax({
		  url: '<?php echo BOOKING_ADMIN_URL;?>ajax/setSlotsOrderby.php?order_by='+column+'&type='+type+"&calendar_id=<?php echo $_GET["calendar_id"]; ?>",
		  success: function(data) {
			  $wbc('#table').hide().html(data).fadeIn(2000);						 
	...
	function delItem(itemId,reservation) {
		if(confirm("<?php echo esc_js(__( 'Are you sure you want to delete this slot?', 'wp-booking-calendar' )); ?>")) {
			$wbc('body').prepend('<div id="sfondo" class="booking_modal_sfondo"><div id="modal_loading" class="booking_modal_loading"><img src="<?php echo BOOKING_ADMIN_URL;?>images/loading.png" border=0 /></div></div>');
			$wbc.ajax({
			  url: '<?php echo BOOKING_ADMIN_URL;?>ajax/delSlotsItem.php?item_id='+itemId+"&reservation="+reservation+"&calendar_id=<?php echo $_GET["calendar_id"]; ?>",
			  success: function(data) {
				  $wbc('#sfondo').remove();
				  $wbc('#table').hide().html(data).fadeIn(2000);
				 goToByScroll("results");
	...
	function editItem(slot) {
		$wbc('#modify_'+slot).html('<a href="javascript:saveItem('+slot+');"><?php echo __( 'Save', 'wp-booking-calendar' ); ?></a>');
	...
	function saveItem(slot) {
		if(Trim($wbc('#slot_date_'+slot).val()) != '' && Trim($wbc('#slot_time_from_hour_'+slot).val()) != '' && Trim($wbc('#slot_time_from_minute_'+slot).val()) != '' && Trim($wbc('#slot_time_to_hour_'+slot).val()) != '' && Trim($wbc('#slot_time_to_minute_'+slot).val()) != '') {
			...
			$wbc.ajax({
			  url: '<?php echo BOOKING_ADMIN_URL;?>ajax/saveSlot.php?item_id='+slot+"&date="+$wbc('#date_value_'+slot).val()+"&time_from="+$wbc('#slot_time_from_hour_'+slot).val()+":"+$wbc('#slot_time_from_minute_'+slot).val()+":"+$wbc('#slot_time_from_ampm_'+slot).val()+"&time_to="+$wbc('#slot_time_to_hour_'+slot).val()+":"+$wbc('#slot_time_to_minute_'+slot).val()+":"+$wbc('#slot_time_to_ampm_'+slot).val()+"&price="+newprice+"&av="+newav+"&avmax="+newavmax,
			  success: function(data) {
				  if(data == 0) {
					  alert("<?php echo esc_js(__( 'There\'s another slot for the same date and time. Change values', 'wp-booking-calendar' )); ?>");
				  } else {
					  ...
					  $wbc('#modify_'+slot).html('<a href="javascript:editItem('+slot+');"><?php echo __( 'Modify', 'wp-booking-calendar' ); ?></a>');
			...
			alert("<?php echo esc_js(__( 'Slot times values are not correct', 'wp-booking-calendar' )); ?>");
		}
	}
	...
	function checkModifyData(frm) {
		...
		with(frm) {
			if(date_from_edit.value == '') {
				...
				alert("<?php echo esc_js(__( 'Select a date to', 'wp-booking-calendar' )); ?>");
				return false;
			} else if(checkEditTimes() == 1) {
				alert("<?php echo esc_js(__( 'New time range values are not correct', 'wp-booking-calendar' )); ?>");
				return false;
			} else {
				return true;
			}
...
<a href="javascript:openSection('search_div');" class="booking_manage_slot_box_title booking_border_1 booking_border_solid booking_border_ccc booking_margin_t_20"><?php echo __( 'Search Time Slots', 'wp-booking-calendar' ); ?></a>

<div class="booking_padding_5 booking_width_60p booking_bg_f6f" id="search_div" style="display:none !important">
    <div class="booking_font_14 booking_margin_t_20"><?php echo __( 'Use the following filters to search created time slots', 'wp-booking-calendar' ); ?></div>
    
    <!-- select filter by date -->
    <div class="booking_margin_t_20 booking_font_14">
        <div class="booking_float_left booking_height_30 booking_line_30"><label for="search_date"><?php echo __( 'Filter by date:', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_l_10">
            <select name="search_date" id="search_date" onchange="javascript:showFilters(this.options[this.selectedIndex].value);">
                <option value="0"><?php echo __( 'choose a date', 'wp-booking-calendar' ); ?></option>
                <option value="1"><?php echo __( 'Single day', 'wp-booking-calendar' ); ?></option>
    ...
    <div id="filters" style="display:none !important">
        ...
        <div class="booking_float_left booking_height_30 booking_line_30 booking_margin_t_20"><label for="date_from" id="label_from"><?php echo __( 'From', 'wp-booking-calendar' ); ?></label></div>
        ...
        <div id="date_to_filter" style="display:none !important">
            <div class="booking_float_left booking_height_30 booking_line_30 booking_margin_l_10 booking_margin_t_20"><label for="date_to"><?php echo __( 'To', 'wp-booking-calendar' ); ?></label></div>
            <div class="booking_float_left booking_margin_l_10 booking_margin_t_20">
        ...
        <div class="booking_margin_t_20" style="display:none !important" id="weekdays_filter" >
            <div class="booking_float_left booking_height_30 booking_line_30"><label for="slot_week_day"><?php echo __( 'Days', 'wp-booking-calendar' ); ?></label></div>
            <div class="booking_float_left booking_margin_l_10">
                <select name="slot_week_day" id="slot_week_day">
                    <option value="0"><?php echo __( 'All', 'wp-booking-calendar' ); ?></option>
                    <option value="1"><?php echo __( 'Mondays', 'wp-booking-calendar' ); ?></option>
                    <option value="2"><?php echo __( 'Tuesdays', 'wp-booking-calendar' ); ?></option>
                    <option value="3"><?php echo __( 'Wednesdays', 'wp-booking-calendar' ); ?></option>
                    <option value="4"><?php echo __( 'Thursdays', 'wp-booking-calendar' ); ?></option>
                    <option value="5"><?php echo __( 'Fridays', 'wp-booking-calendar' ); ?></option>
                    <option value="6"><?php echo __( 'Saturdays', 'wp-booking-calendar' ); ?></option>
                    <option value="7"><?php echo __( 'Sundays', 'wp-booking-calendar' ); ?></option>
                </select>
            </div>
        </div>
        <div class="booking_cleardiv"></div>
        ...
        <div class="booking_float_left booking_height_30 booking_line_30 booking_margin_t_20"><label for="time_from"><?php echo __( 'Time From', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_l_10 booking_margin_t_20">
        ...
        <div class="booking_float_left booking_height_30 booking_line_30 booking_margin_t_20 booking_margin_l_10"><label for="time_to"><?php echo __( 'Time To', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_l_10 booking_margin_t_20">
            <div>
            ...
            <div class="booking_font_12 booking_float_right booking_mark_666 booking_margin_t_5"><?php echo __( 'Leave empty if you don\'t want a range period of time', 'wp-booking-calendar' ); ?></div>
        </div>
        ...
        <div class="booking_margin_t_20">
        	<input type="button" name="saveunpublish" onclick="javascript:filterSlots();" value="<?php echo __( 'Search', 'wp-booking-calendar' ); ?>" class="booking_bg_693 booking_admin_button booking_green_button booking_mark_fff">
...
<a href="javascript:openSection('modify_div');" class="booking_manage_slot_box_title booking_border_1 booking_border_solid booking_border_ccc booking_margin_t_20"><?php echo __( 'Modify Time Slots', 'wp-booking-calendar' ); ?></a>

<div class="booking_padding_5 booking_width_60p booking_bg_f6f" id="modify_div" style="display:none !important">

    <div class="booking_float_left booking_margin_t_20 booking_height_30 booking_line_30"><label for="form_action"><?php echo __( 'Action:', 'wp-booking-calendar' ); ?></label></div>
    <div class="booking_float_left booking_margin_l_10 booking_margin_t_20">
        <select name="form_action" id="form_action" onchange="javascript:showForm(this.options[this.selectedIndex].value);">
            ...
            <option value="1"><?php echo __( 'modify', 'wp-booking-calendar' ); ?></option>
            <option value="2"><?php echo __( 'delete', 'wp-booking-calendar' ); ?></option>
        </select>
    </div>
    <div class="booking_cleardiv"></div>
    
    
    <form style="display:none !important" name="modify_slots" method="post" action="<?php echo BOOKING_ADMIN_URL;?>ajax/checkEditSlots.php" target="frame_submit" id="modify_form" onsubmit="return checkModifyData(this);">
        <input type="hidden" name="calendar_id" value="<?php echo $_GET["calendar_id"]; ?>" />
        
        <div class="booking_float_left booking_margin_t_20 booking_height_30 booking_line_30"><label for="slot_hour_edit"><?php echo __( 'Slot:', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_l_10 booking_margin_t_20">
        ...
        <!-- select weekdays -->
        <div class="booking_float_left booking_margin_t_20 booking_height_30 booking_line_30"><label for="slot_week_day_edit"><?php echo __( 'Weekdays:', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_l_10 booking_margin_t_20">
        ...
        <!-- from to -->
        <div class="booking_float_left booking_height_30 booking_line_30 booking_margin_t_20"><label for="date_from_edit"><?php echo __( 'From', 'wp-booking-calendar' ); ?></label></div>
        ...
        <div class="booking_float_left booking_margin_l_10 booking_margin_t_20"><label for="date_to_edit"><?php echo __( 'To', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_t_20 booking_margin_l_10">
        ...
        <div class="booking_cleardiv"></div>
         ...
         <!-- filter by range time -->
         <div class="booking_float_left booking_margin_t_20 booking_height_30 booking_line_30"><label for="time_from"><?php echo __( 'New Time From', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_t_20 booking_margin_l_10">
        ...
        <div class="booking_float_left booking_margin_l_10 booking_margin_t_20 booking_height_30 booking_line_30"><label for="time_to"><?php echo __( 'New Time To', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_t_20 booking_margin_l_10">
        ...
        if($bookingSettingObj->getPaypalDisplayPrice() == 1) {
            ?>
            <div class="booking_float_left booking_height_30 booking_line_30 booking_margin_t_20"><label for="slot_price_edit"><?php echo __( 'New Price', 'wp-booking-calendar' ); ?></label></div>
        ...
        if($bookingSettingObj->getSlotsUnlimited() == 2) {
            ?>
            <div class="booking_float_left booking_height_30 booking_line_30 booking_margin_t_20"><label for="slot_price_edit"><?php echo __( 'New slot seats', 'wp-booking-calendar' ); ?></label></div>
            ...
            <div class="booking_cleardiv"></div>
            <div class="booking_float_left booking_height_30 booking_line_30 booking_margin_t_20"><label for="slot_price_edit"><?php echo __( 'New maximum bookable slot seats', 'wp-booking-calendar' ); ?></label></div>
        ...
        ?>
            ...
            <div class="input_boxes_container">
                ...
                <div class="input_title" style="margin-right: 66px;"><label for="slot_price_edit"><?php echo __( 'New text', 'wp-booking-calendar' ); ?></label></div>
       ...
       */
	   ?>
        ...
        <!-- search -->
        <div class="booking_margin_t_20"><input type="submit" class="booking_bg_693 booking_admin_button booking_green_button booking_mark_fff" id="edit_button" name="saveunpublish" value="<?php echo __( 'Modify', 'wp-booking-calendar' ); ?>"></div>
        <!-- loader -->
    ...
    <form style="display:none !important" name="delete_slots" method="post" action="<?php echo BOOKING_ADMIN_URL;?>ajax/checkSlots.php" target="frame_submit" tmt:validate="true" id="delete_form">
        <input type="hidden" name="calendar_id" value="<?php echo $_GET["calendar_id"]; ?>" />
        <div class="booking_margin_t_20 booking_float_left booking_height_30 booking_line_30"><label for="slot_hour_delete"><?php echo __( 'Slot:', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_l_10 booking_margin_t_20">
        ...
        <!-- select weekdays -->
        <div class="booking_margin_t_20 booking_float_left booking_height_30 booking_line_30"><label for="slot_week_day_delete"><?php echo __( 'Weekdays:', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_l_10 booking_margin_t_20">
        ...
        <div class="booking_float_left booking_margin_t_20 booking_height_30 booking_line_30"><label for="date_from_delete"><?php echo __( 'From', 'wp-booking-calendar' ); ?></label></div>
        ...
        <div class="booking_float_left booking_margin_t_20 booking_height_30 booking_line_30 booking_margin_l_10"><label for="date_to_edit"><?php echo __( 'To', 'wp-booking-calendar' ); ?></label></div>
        <div class="booking_float_left booking_margin_t_20 booking_margin_l_20  booking_margin_l_10">
            ...
            <div class="booking_font_12 booking_margin_t_5 booking_float_right"><?php echo __( 'Leave empty if you don\'t want a range period of time', 'wp-booking-calendar' ); ?></div>
        </div>
            
        <div class="booking_cleardiv"></div>
        
        
        <div class="booking_margin_t_20"><input type="submit" class="booking_bg_693 booking_admin_button booking_green_button booking_mark_fff" id="del_button" name="saveunpublish" value="<?php echo __( 'Delete', 'wp-booking-calendar' ); ?>"></div>
...
<div class="booking_margin_t_20 booking_bg_f6f booking_border_1 booking_border_solid booking_border_ccc">
	<a onclick="javascript:delItems('manage_slots','slots[]','delSlots','<?php echo __( 'Are you sure to delete the selected items?', 'wp-booking-calendar' );?>','<?php echo __( 'No items selected', 'wp-booking-calendar' ); ?>')" class="booking_float_right booking_pointer booking_padding_10"><?php echo __( 'Delete', 'wp-booking-calendar' );?></a>
    <div class="booking_cleardiv"></div>
</div>
<div class="booking_cleardiv"></div>
...
<form name="manage_slots" action="" method="post" style="display:inline;">
	...
	<div id="table" class="booking_margin_t_20">
    	...
    	<div class="booking_border_b_1 booking_border_solid booking_border_ccc booking_bg_f6f booking_height_30 booking_line_30">
			...
			<div class="booking_float_left booking_width_10p"><?php echo __( 'Date', 'wp-booking-calendar' );?>&nbsp;<a href="javascript:orderby('<?php echo $_SESSION["orderbySlotsDate"]; ?>');"><img src="<?php echo BOOKING_ADMIN_URL;?>images/orderby_<?php echo $_SESSION["orderbySlotsDate"];?>.gif" border=0 /></a></div>
			
			<div class="booking_float_left booking_width_10p"><?php echo __( 'Time From', 'wp-booking-calendar' );?>&nbsp;<a href="javascript:orderby('<?php echo $_SESSION["orderbySlotsTime"]; ?>');"><img src="<?php echo BOOKING_ADMIN_URL;?>images/orderby_<?php echo $_SESSION["orderbySlotsTime"];?>.gif" border=0 /></a></div>
			
            <div class="booking_float_left booking_width_20p"><?php echo __( 'Time To', 'wp-booking-calendar' );?></div>
			
			<div class="booking_float_left booking_width_20p"><?php echo __( 'Special Text', 'wp-booking-calendar' );?></div>
			
            <div class="booking_float_left booking_width_5p"><?php echo __( 'Price', 'wp-booking-calendar' );?></div>
			
			<div class="booking_float_left booking_width_5p"><?php echo __( 'Seats', 'wp-booking-calendar' );?></div>
			
			<div class="booking_float_left booking_width_5p"><?php echo __( 'Reserved', 'wp-booking-calendar' );?></div>
