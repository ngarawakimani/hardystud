...
<div class="booking_padding_20 booking_font_14 booking_line_percent booking_bg_fff">
            ...
            <script>
                ...
                function delItem(itemId) {
                    if(confirm("<?php echo esc_js(__('Are you sure you want to delete this category? All calendars, slots, holidays and reservations related to it will be deleted too','wp-booking-calendar')); ?>")) {
                        $wbc.ajax({
                          url: '<?php echo BOOKING_ADMIN_URL; ?>ajax/delCategoryItem.php?item_id='+itemId,
                ...
                function publishCategory(itemId) {
                    if(confirm("<?php echo esc_js(__('Are you sure you want to publish this category?','wp-booking-calendar')); ?>")) {
                        $wbc.ajax({
                          url: '<?php echo BOOKING_ADMIN_URL;?>ajax/publishCategory.php?category_id='+itemId,
                          success: function(data) {
                              $wbc('#publish_'+itemId).html('<a href="javascript:unpublishCategory('+itemId+');"><img src="<?php echo BOOKING_ADMIN_URL;?>images/icons/published.png" border=0 /></a>');
                ...
                function unpublishCategory(itemId) {
                    if(confirm("<?php echo esc_js(__('Are you sure you want to unpublish this category?','wp-booking-calendar')); ?>")) {
                        $wbc.ajax({
                          url: '<?php echo BOOKING_ADMIN_URL;?>ajax/unpublishCategory.php?category_id='+itemId,
                          success: function(data) {
                              $wbc('#publish_'+itemId).html('<a href="javascript:publishCategory('+itemId+');"><img src="<?php echo BOOKING_ADMIN_URL;?>images/icons/unpublished.png" border=0 /></a>');
                ...
                function addCategory() {
                    if(Trim($wbc('#category_name').val())!= '') {
                        $wbc.ajax({
                          url: '<?php echo BOOKING_ADMIN_URL; ?>ajax/addCategory.php?category_name='+$wbc('#category_name').val(),
                          success: function(data) {
                              $wbc('#booking_table').hide().html(data).fadeIn(2000);							 
                        ...
                        alert("<?php echo esc_js(__('Insert a name for the category','wp-booking-calendar')); ?>");
                    }
                }
                ...
                function saveItem(category) {
                    if(Trim($wbc('#category_name_'+category).val()) != '') {
						$wbc('#modify_'+category).parent().append('<img id="small_save_loader" src="<?php echo BOOKING_ADMIN_URL;?>images/loading.gif" style="display: table-cell;vertical-align:middle" border=0>');
                        $wbc.ajax({
                          url: '<?php echo BOOKING_ADMIN_URL;?>ajax/saveCategory.php?item_id='+category+"&name="+$wbc('#category_name_'+category).val(),
                          success: function(data) {
                              ...
                              $wbc('#modify_'+category).html('<a href="javascript:editItem('+category+');"><?php echo esc_js(__('Modify name','wp-booking-calendar')); ?></a>');
                ...
                function setDefaultCategory(category) {
                    $wbc.ajax({
                      url: '<?php echo BOOKING_ADMIN_URL;?>ajax/setDefaultCategory.php?category_id='+category,
                      success: function(data) {
