...
class wp_booking_calendar_lists {	
	...
	public function getReservationsList($filter,$order_by,$calendar_id) {
		...
		$reservationsQry = $wpdb->get_results("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation r INNER JOIN ".$wpdb->base_prefix.$blog_prefix."booking_slots s ON s.slot_id=r.slot_id WHERE r.calendar_id = '".$calendar_id."' AND s.calendar_id = '".$calendar_id."' AND reservation_fake = 0 ".$filter." ".$order_by);
	...
	public function getUsersReservationsList($filter,$order_by,$user_id) {
		...
		$arrayReservations = Array();
		$reservationsQry = $wpdb->get_results("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation r INNER JOIN ".$wpdb->base_prefix.$blog_prefix."booking_slots s ON s.slot_id=r.slot_id WHERE r.wordpress_user_id = '".$user_id."' AND reservation_fake = 0 ".$filter." ".$order_by);
	...
	public function getCalendarsResList() {
		...
		$arrayCalendars = Array();
		$calendarsQry = $wpdb->get_results("SELECT c.*, COUNT(r.reservation_id) as tot_reservation FROM ".$wpdb->base_prefix.$blog_prefix."booking_calendars c LEFT JOIN ".$wpdb->base_prefix.$blog_prefix."booking_reservation r ON r.calendar_id = c.calendar_id AND r.reservation_fake = 0  GROUP BY c.calendar_id ORDER BY c.calendar_order");
