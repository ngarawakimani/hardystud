	...
	public function addSlot() {
		...
		if(count($resultDate)>0) {
			...
			do {
				...
				if($i>0) {
					$slug = "slots_from_".str_replace("-","",$resultDate[0])."_".str_replace(":","",$resultTime[0]["time_from"])."_to_".str_replace("-","",$resultDate[count($resultDate)-1])."_".str_replace(":","",$resultTime[count($resultTime)-1]["time_to"])."-".$i;
				} else {
					$slug = "slots_from_".str_replace("-","",$resultDate[0])."_".str_replace(":","",$resultTime[0]["time_from"])."_to_".str_replace("-","",$resultDate[count($resultDate)-1])."_".str_replace(":","",$resultTime[count($resultTime)-1]["time_to"]);
				}
				$slug_def = $slug;

				$checkQry=$wpdb->query($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots_bundles WHERE bundle_name=%s",$slug));
				$numrows = $checkQry;
			...
			$wpdb->query($wpdb->prepare("INSERT INTO ".$wpdb->base_prefix.$blog_prefix."booking_slots_bundles(bundle_name,bundle_text) VALUES(%s,%s)",$slug_def,$slot_special_text));
			$bundle_id = $wpdb->insert_id;
			
			$package = array(
			 'kind' => 'WP Booking Calendar',
			 'name' => 'slots',
			 'title' => 'WP Booking Calendar - Slots'
			 );
			
			do_action( 'wpml_register_string', $slot_special_text, $slug_def, $package, "Slots from ".$resultDate[0]." ".$resultTime[0]["time_from"]." to ".$resultDate[count($resultDate)-1]." ".$resultTime[count($resultTime)-1]["time_to"]." text", 'LINE' );
				...
				if($holidaysQry==0) {
					for($j=0;$j<count($resultTime);$j++) {
						...
						if($checkQry==0) {
							...
							$wpdb->query($wpdb->prepare("INSERT INTO ".$wpdb->base_prefix.$blog_prefix."booking_slots(slot_special_text,slot_special_mode,slot_date,slot_time_from,slot_time_to,slot_price,slot_av,slot_av_max,slot_active,calendar_id,bundle_id,slot_discount_price,slot_perc_price,slot_show_price) VALUES(%s,%d,%s,%s,%s,%s,%d,%d,%d,%d,%d,%s,%s,%d)",$slot_special_text,$slot_special_mode,$resultDate[$i],$resultTime[$j]["time_from"],$resultTime[$j]["time_to"],$price,$slot_av,$slot_av_max,1,$_POST["calendar_id"],$bundle_id,$discount_price,$perc_price,$_POST["slot_show_price"]));
		...
		/*check if it passed, if not, remove bundle and string*/
		if($count == 0) {
			$wpdb->query("DELETE FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots_bundles WHERE bundle_id=".$bundle_id);
            if(function_exists('icl_unregister_string')) {
                icl_unregister_string('wp-booking-calendar-slots', $slug_def);
            }
		}
