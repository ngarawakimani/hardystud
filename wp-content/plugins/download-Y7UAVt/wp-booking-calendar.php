...
Text Domain: wp-booking-calendar
...
if(isset($sitepress)) {
    ...
    $current_domain = $domains[$sitepress->get_current_language()];
    $different = false;
    if($domains) {
        ...
        if($different) {
            ...
            define('BOOKING_PUBLIC_URL', $home_url.'wp-content/plugins/wp-booking-calendar/public/');
            define('BOOKING_ADMIN_URL', $home_url.'wp-content/plugins/wp-booking-calendar/admin/');
        } else {
            define('BOOKING_PUBLIC_URL', plugins_url('wp-booking-calendar/public/'));
            define('BOOKING_ADMIN_URL', plugins_url('wp-booking-calendar/admin/'));
        }
    } else {
        define('BOOKING_PUBLIC_URL', plugins_url('wp-booking-calendar/public/'));
        define('BOOKING_ADMIN_URL', plugins_url('wp-booking-calendar/admin/'));
...
function wp_booking_calendar_plugin_api_call($def, $action, $args) {
	global $wp_booking_plugin_slug,$wp_booking_plugin_slug_update, $wp_booking_api_url, $wp_version;
	if (!isset($args->slug) || ($args->slug != $wp_booking_plugin_slug_update))
		return false;
...
function booking_calendar_admin_scripts() {
	if (wp_booking_calendar_is_my_plugin_screen()) {  
       ...
       wp_enqueue_script('jquery');
		wp_enqueue_style('wp-booking-admin-style-css',BOOKING_ADMIN_URL.'css/style.css');
		wp_enqueue_style('wp-booking-admin-jquery-lightbox-css',BOOKING_ADMIN_URL.'css/jquery.lightbox-0.5.css');
		wp_enqueue_style('wp-booking-admin-jquery-ui-css',BOOKING_ADMIN_URL.'css/jquery-ui.css');
		wp_enqueue_script('wp-booking-admin-lib-js',BOOKING_ADMIN_URL.'js/lib.js');
		wp_enqueue_script('wp-booking-admin-jquery-lightbox-js',BOOKING_ADMIN_URL.'js/jquery.lightbox-0.5.js',array('jquery'));
		wp_enqueue_script('wp-booking-admin-tmt_core-js',BOOKING_ADMIN_URL.'js/tmt_libs/tmt_core.js');
		wp_enqueue_script('wp-booking-admin-tmt_form-js',BOOKING_ADMIN_URL.'js/tmt_libs/tmt_form.js');
		wp_enqueue_script('wp-booking-admin-tmt_validator-js',BOOKING_ADMIN_URL.'js/tmt_libs/tmt_validator.js');
		wp_enqueue_script('wp-booking-admin-jquery-ui-custom-js',BOOKING_ADMIN_URL.'js/jquery-ui-1.10.3.custom.js',array('jquery','wp-booking-admin-jquery-ui-js'));
		wp_enqueue_script('wp-booking-admin-jquery-ui-js',BOOKING_ADMIN_URL.'js/jquery-ui-i18n.js',array('jquery'));
		wp_enqueue_script('wp-booking-admin-jquery-simple-color-js',BOOKING_ADMIN_URL.'js/jquery.simple-color.js',array('jquery'));
    } 
...
function booking_calendar_public_scripts() {
    ...
    wp_enqueue_script('jquery');
    wp_enqueue_style('wp-booking-calendar-public-font-awesome-css',BOOKING_PUBLIC_URL.'css/font-awesome/css/font-awesome.min.css');
	wp_enqueue_style('wp-booking-calendar-public-mainStyle-css',BOOKING_PUBLIC_URL.'css/mainstyle.css');
	
	wp_enqueue_style('wp-booking-calendar-public-ie-style', BOOKING_PUBLIC_URL.'css/ie.css');

    wp_enqueue_style('wp-booking-calendar-custom-google-font',$bookingSettingObj->getGoogleFontLinkCode());
    global $wp_styles;
	...
	if($bookingSettingObj->getRecaptchaEnabled() == "1") {
        if($bookingSettingObj->getRecaptchaVersion() == 1) {
            wp_register_script('wp-booking-calendar-public-recaptcha','http://www.google.com/recaptcha/api/js/recaptcha_ajax.js');
        } else {
            wp_register_script('wp-booking-calendar-public-recaptcha','https://www.google.com/recaptcha/api.js');
            add_filter( 'script_loader_tag', function ( $tag, $handle ) {

                if ( 'wp-booking-calendar-public-recaptcha' !== $handle )
                    return $tag;
	...
	wp_enqueue_script('wp-booking-calendar-public-bxslider-js',BOOKING_PUBLIC_URL.'js/jquery.bxSlider.min.js',array('jquery'));
	wp_enqueue_script('wp-booking-calendar-public-tmt_core-js',BOOKING_PUBLIC_URL.'js/tmt_libs/tmt_core.js');
	wp_enqueue_script('wp-booking-calendar-public-tmt_form-js',BOOKING_PUBLIC_URL.'js/tmt_libs/tmt_form.js');
	wp_enqueue_script('wp-booking-calendar-public-tmt_validator-js',BOOKING_PUBLIC_URL.'js/tmt_libs/tmt_validator.js');
	wp_enqueue_script('wp-booking-calendar-public-lib-js',BOOKING_PUBLIC_URL.'js/lib.js',array('jquery'));
	wp_enqueue_script('wp-booking-calendar-public-calendar_js',BOOKING_PUBLIC_URL.'js/wach.calendar.js',array('jquery'));
	
	wp_localize_script( 'wp-booking-calendar-public-calendar_js', 'WPBookingCalendarSettings', array(
	  	'path' => BOOKING_PUBLIC_URL,
        ...
        'day_white_line2_bg' => $bookingSettingObj->getDayWhiteLine2Bg(),
        'day_white_line2_bg_hover' => $bookingSettingObj->getDayWhiteLine2BgHover(),
...
function wp_booking_calendar_is_my_plugin_screen() {  
	...
	$arrScreens = array('toplevel_page_wp-booking-calendar-welcome',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-welcome',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-settings',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-reservations',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-mails',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-form',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-style',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-categories',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-texts',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-orders',sanitize_title( __('Booking Calendar','wp-booking-calendar')).'_page_wp-booking-calendar-contact-admin');
...
function wp_booking_calendar_admin_menu() {
	...
	add_menu_page(__('Booking Calendar','wp-booking-calendar'), __('Booking Calendar','wp-booking-calendar'), 'wbc_user_orders', 'wp-booking-calendar-welcome', 'wp_booking_calendar_admin_view_welcome_page',BOOKING_ADMIN_URL.'images/icon.png' );
	
	@add_submenu_page('wp-booking-calendar-welcome', __('Settings','wp-booking-calendar'), __('Settings','wp-booking-calendar'), 'wbc_view_slots', 'wp-booking-calendar-settings', 'wp_booking_calendar_admin_view_settings');	
	@add_submenu_page('wp-booking-calendar-welcome', __('Bg and Colors','wp-booking-calendar'), __('Bg and Colors','wp-booking-calendar'), 'wbc_view_slots', 'wp-booking-calendar-style', 'wp_booking_calendar_admin_view_styles');	
	@add_submenu_page('wp-booking-calendar-welcome', __('Manage Categories','wp-booking-calendar'), __('Manage Categories','wp-booking-calendar'), 'wbc_view_slots', 'wp-booking-calendar-categories', 'wp_booking_calendar_admin_view_categories_list');
	@add_submenu_page('wp-booking-calendar-welcome', __('Manage Calendars','wp-booking-calendar'), __('Manage Calendars','wp-booking-calendar'), 'wbc_view_slots', 'wp-booking-calendar', 'wp_booking_calendar_admin_view_calendars_list');	
	@add_submenu_page('wp-booking-calendar-welcome', __('Reservations','wp-booking-calendar'), __('Reservations','wp-booking-calendar'), 'wbc_view_slots', 'wp-booking-calendar-reservations', 'wp_booking_calendar_admin_view_reservations_list');
	@add_submenu_page('wp-booking-calendar-welcome', __('Manage Mail','wp-booking-calendar'), __('Manage Mail','wp-booking-calendar'), 'wbc_view_slots', 'wp-booking-calendar-mails', 'wp_booking_calendar_admin_view_mails_list');
	@add_submenu_page('wp-booking-calendar-welcome', __('Form management','wp-booking-calendar'), __('Form management','wp-booking-calendar'), 'wbc_view_slots', 'wp-booking-calendar-form', 'wp_booking_calendar_admin_view_form');	
	@add_submenu_page(null, __('Text management','wp-booking-calendar'), __('Text management','wp-booking-calendar'), 'wbc_view_slots', 'wp-booking-calendar-texts', 'wp_booking_calendar_admin_view_texts');	
	@add_submenu_page('wp-booking-calendar-welcome', __('Your reservations','wp-booking-calendar'), __('Your reservations','wp-booking-calendar'), 'wbc_user_orders', 'wp-booking-calendar-orders', 'wp_booking_calendar_admin_view_orders');	
	@add_submenu_page('wp-booking-calendar-welcome', __('Contact Administrator','wp-booking-calendar'), __('Contact Administrator','wp-booking-calendar'), 'wbc_user_orders', 'wp-booking-calendar-contact-admin', 'wp_booking_calendar_admin_contact_admin');	
...
$linkToUpload='';
if((get_option('wbc_show_text_update_admin')=='1' || get_option('wbc_show_text_update_public')=='1') && strtolower(get_current_user_role()) == 'administrator') {
...
$linkToImport='';
if(get_option('wbc_show_text_import_admin')=='1' && strtolower(get_current_user_role()) == 'administrator') {
	$linkToImport = '<div class="updated"><div><strong>Booking Calendar WP Plugin</strong></div><div class="booking_float_left">Update your texts from previous version using the automatic import <a href="admin.php?page=wp-booking-calendar-texts&import=1">clicking here</a>!</div><div class="booking_float_right"><a href="admin.php?page=wp-booking-calendar-texts&import=0">Don\'t show it again</a></div><div class="booking_cleardiv"></div></div>';
}
...
function wp_booking_calendar_public_show_calendar($calendar_id = 0,$category_id = 0) {
    ...
    wp_enqueue_script('wp-booking-calendar-public-recaptcha');
    if($wpdb->get_var( $wpdb->prepare("SELECT config_value FROM ".$wpdb->base_prefix.$blog_prefix."booking_config WHERE config_name = %s", "reservation_confirmation_mode") )>0) {
