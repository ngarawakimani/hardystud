...
/*preparing week days*/
$weekdays=Array();
$weekdays[0] = __( 'Sunday', 'wp-booking-calendar' );
$weekdays[1] = __( 'Monday', 'wp-booking-calendar' );
$weekdays[2] = __( 'Tuesday', 'wp-booking-calendar' );
$weekdays[3] = __( 'Wednesday', 'wp-booking-calendar' );
$weekdays[4] = __( 'Thursday', 'wp-booking-calendar' );
$weekdays[5] = __( 'Friday', 'wp-booking-calendar' );
$weekdays[6] = __( 'Saturday', 'wp-booking-calendar' );
...
if($prev == '' || $_GET["month"] > $prevMonth || date("Ymd",$prev) < $bookfromdate) {
	?>
    <div class="booking_float_left booking_height_20 booking_line_20 booking_mark_ccc booking_font_16"><?php echo __( 'Prev day', 'wp-booking-calendar' ); ?></div>
    <?php
} else {
	?>
	<div class="booking_float_left booking_height_20 booking_line_20"><a href="javascript:getBookingForm(<?php echo $prevYear; ?>,<?php echo $prevMonth; ?>,<?php echo $prevDay; ?>, <?php echo $_GET["calendar_id"]; ?>,'<?php echo $bookingSettingObj->getRecaptchaPublicKey(); ?>');" class="booking_mark_333 booking_no_decoration booking_hover_567 booking_font_16"><?php echo __( 'Prev day', 'wp-booking-calendar' ); ?></a></div>
...
if($next == '' || $_GET["month"] < $nextMonth || (date("Ymd",$next) > $booktodate && $bookingSettingObj->getBookTo()>0)) {
	?>
    <div class="booking_float_left booking_height_20 booking_line_20 booking_mark_ccc booking_margin_l_2p booking_font_16"><?php echo __( 'Next day', 'wp-booking-calendar' ); ?></div>
    <?php
} else {
	?>
	<div class="booking_float_left booking_height_20 booking_line_20 booking_margin_l_2p"><a href="javascript:getBookingForm(<?php echo $nextYear; ?>,<?php echo $nextMonth; ?>,<?php echo $nextDay; ?>, <?php echo $_GET["calendar_id"]; ?>,'<?php echo $bookingSettingObj->getRecaptchaPublicKey(); ?>');" class="booking_mark_333 booking_no_decoration booking_hover_567 booking_font_16"><?php echo __( 'Next day', 'wp-booking-calendar' ); ?></a></div>
	<?php
}
?>

<!-- close -->
<div class="booking_font_custom booking_float_right text_right"><a href="javascript:closeBookingPage(<?php echo $bookingCalendarObj->getCalendarId(); ?>,'<?php echo $bookingSettingObj->getRecaptchaPublicKey(); ?>',<?php echo $_GET["year"]; ?>,<?php echo $_GET["month"]; ?>);" class="booking_mark_666 close_booking booking_no_decoration"><?php echo __( 'CLOSE', 'wp-booking-calendar' ); ?>&nbsp; X</a></div>
            ...
            <div id="page<?php echo $page; ?>">
                ...
                <div class="booking_float_left">
                    ...
                    foreach($arraySlots as $slotId => $slot) {
					  ...
					  ?>
                      	<!-- checkbox -->
                        <div class="booking_float_left booking_margin_t_5"><input type="checkbox" name="reservation_slot[]" value="<?php echo $slotId; ?>" tmt:minchecked="1" tmt:message="<?php echo __( 'Select at least one time slot', 'wp-booking-calendar' ); ?>" <?php echo $disabled; ?> onclick="<?php echo $onclick; ?>" style="-webkit-appearance: checkbox !important;display:block !important;width:14px !important"/></div>
						...
						if($bookingSettingObj->getSlotsUnlimited() == 2) {
                            ...
                            <div class="booking_float_left booking_margin_l_15 booking_height_30 booking_line_30">
                            	<?php echo __( 'Select seats', 'wp-booking-calendar' ); ?>:&nbsp;
                                ...
                                if($bookingSettingObj->getPaypal()==1 && $bookingSettingObj->getPaypalAccount() != '' && $bookingSettingObj->getPaypalLocale() != '' && $bookingSettingObj->getPaypalCurrency() != '') {
                                    if($slot["slot_discount_price"]>0 || $slot["slot_perc_price"]>0) {
                                        switch($slot["slot_show_price"]) {
                                            ...
                                            case 1:
                                                if($slot["slot_discount_price"]>0) {
                                                    echo money_format('%!.2n',$slot["slot_discount_price"]); ?>&nbsp;<?php echo $bookingSettingObj->getPaypalCurrency();
                                                } else if($slot["slot_perc_price"]>0) {
                                                    echo money_format('%!.2n',($slot["slot_price"]/100*$slot["slot_perc_price"])); ?>&nbsp;<?php echo $bookingSettingObj->getPaypalCurrency();
                                                }
                                                break;
                                            case 2:
                                                echo '<span class="booking_red_strike">'.money_format('%!.2n',$slot["slot_price"]); ?>&nbsp;<?php echo $bookingSettingObj->getPaypalCurrency().'</span><span>&nbsp;';
                                                if($slot["slot_discount_price"]>0) {
                                                    echo money_format('%!.2n',$slot["slot_discount_price"]); ?>&nbsp;<?php echo $bookingSettingObj->getPaypalCurrency();
                                                } else if($slot["slot_perc_price"]>0) {
                                                    echo money_format('%!.2n',($slot["slot_price"]/100*$slot["slot_perc_price"])); ?>&nbsp;<?php echo $bookingSettingObj->getPaypalCurrency();
                                        ...
                                        if($slot["slot_price"]>0) {
                                            ...
                                            echo __( 'Free', 'wp-booking-calendar' );
                                        }
                                    }
                                } else {
                                    if($slot["slot_price"]>0) {
...
?>
|<?php echo __( 'Insert valid verification code', 'wp-booking-calendar' ); ?>
