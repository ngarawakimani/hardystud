...
class wp_booking_calendar_public_lists {	
	...
	public function getSlotsList($filter,$order_by,$calendar_id) {
		...
		$slotsQry = $wpdb->get_results($wpdb->prepare("SELECT s.*,b.bundle_name FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots s LEFT JOIN ".$wpdb->base_prefix.$blog_prefix."booking_slots_bundles b ON b.bundle_id = s.bundle_id WHERE s.slot_active = %d AND s.calendar_id = %d ".$filter." ".$order_by,1,$calendar_id));
		...
		for($i=0;$i<count($slotsQry);$i++) {
			...
			if($slotsQry[$i]->bundle_name == '') {
				$arraySlots[$slotsQry[$i]->slot_id]["slot_special_text"] = stripslashes($slotsQry[$i]->slot_special_text);
			} else {
				$translated_string = apply_filters( 'wpml_translate_single_string', stripslashes($slotsQry[$i]->slot_special_text), "wp-booking-calendar-slots", $slotsQry[$i]->bundle_name );
				$arraySlots[$slotsQry[$i]->slot_id]["slot_special_text"] = $translated_string;
			}
			$arraySlots[$slotsQry[$i]->slot_id]["slot_special_mode"] = $slotsQry[$i]->slot_special_mode;
			
			$reservation=$wpdb->query($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE slot_id = %d AND reservation_cancelled = %d AND reservation_fake = %d",$slotsQry[$i]->slot_id,0,0));
			if($reservation == 0) {
				$reservation = "NO";
			} else {
				$reservation = "YES";
	...
	public function getReservationsList($filter,$order_by,$calendar_id) {
		...
		$reservationsQry = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation r INNER JOIN ".$wpdb->base_prefix.$blog_prefix."booking_slots s ON s.slot_id=r.slot_id WHERE r.calendar_id = %d AND s.calendar_id = %d  AND r.reservation_fake = 0 ".$filter." ".$order_by,$calendar_id,$calendar_id));
	...
	public function getCalendarsList($order_by,$category_id = 0) {
		...
		for($i=0;$i<count($calendarsQry);$i++) {
			...
			$translated_string = apply_filters( 'wpml_translate_single_string', stripslashes($calendarsQry[$i]->calendar_title), 'wp-booking-calendar-calendars', 'calendar_'.$calendarsQry[$i]->calendar_id );
			$arrayCalendars[$calendarsQry[$i]->calendar_id]["calendar_title"] = $translated_string;
	...
	public function getSlotsPerDay($year,$month,$daynum, $calendar_id,$bookingSettingObj) {
		...
		if($tot == 0) {
			...
			if($bookingSettingObj->getSlotsUnlimited() != 1 && $bookingSettingObj->getShowSlotsSeats() == 0) {
				for($i=0;$i<count($slotsQry);$i++) {
					$reservationQry = $wpdb->prepare("SELECT SUM(reservation_seats) as res FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE slot_id=%d AND reservation_cancelled = %d AND reservation_fake = %d GROUP BY slot_id",$slotsQry[$i]->slot_id,0,0);
	...
	public function getSlotsPerDayList($year,$month,$day,$calendar_id,$bookingSettingObj) {
		...
		if($year."-".$month."-".$day == date('Y-m-d')) {
			...
			$slotsQry = $wpdb->get_results("SELECT s.*,b.bundle_name FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots s LEFT JOIN ".$wpdb->base_prefix.$blog_prefix."booking_slots_bundles b ON b.bundle_id = s.bundle_id WHERE s.slot_active=1 AND s.slot_date = '".$year."-".$month."-".$day."' AND s.calendar_id=".$calendar_id."  AND REPLACE(s.slot_time_from,':','') >= DATE_FORMAT(NOW(),'%H%i%s') ORDER BY s.slot_time_from");

		} else {
			$slotsQry = $wpdb->get_results($wpdb->prepare("SELECT s.*,b.bundle_name FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots s LEFT JOIN ".$wpdb->base_prefix.$blog_prefix."booking_slots_bundles b ON b.bundle_id = s.bundle_id WHERE s.slot_active = %d AND s.slot_date = %s AND s.calendar_id=%d ORDER BY s.slot_time_from",1,$year."-".$month."-".$day,$calendar_id));
		}
		
		for($i=0;$i<count($slotsQry);$i++) {
			
			if($bookingSettingObj->getSlotsUnlimited() == 0 && $bookingSettingObj->getShowBookedSlots() == 0) {
				$reservationQry=$wpdb->query($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE slot_id=%d AND reservation_cancelled = %d AND reservation_fake = %d",$slotsQry[$i]->slot_id,0,0));
				if($reservationQry==0) {
					$arraySlots[$slotsQry[$i]->slot_id] = Array();
					$arraySlots[$slotsQry[$i]->slot_id]["slot_time_from"] = $slotsQry[$i]->slot_time_from;
                    ...
                    $arraySlots[$slotsQry[$i]->slot_id]["slot_discount_price"] = $slotsQry[$i]->slot_discount_price;
                    $arraySlots[$slotsQry[$i]->slot_id]["slot_perc_price"] = $slotsQry[$i]->slot_perc_price;
                    $arraySlots[$slotsQry[$i]->slot_id]["slot_show_price"] = $slotsQry[$i]->slot_show_price;
				...
				$booked = 0;
				$reservationQry = $wpdb->prepare("SELECT SUM(reservation_seats) as seats FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE slot_id=%d AND reservation_cancelled = %d AND reservation_fake = %d GROUP BY slot_id",$slotsQry[$i]->slot_id,0,0);
				$numrows=$wpdb->query($reservationQry);
	...
	public function getSlotsByReservationsList($reservations) {
		...
		$slotsQry = $wpdb->get_results("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE SHA1(CONCAT(reservation_id, slot_id)) IN (".$listReservations.")");
	...
	public function getCustomerDataList($reservations) {
		...
		$slotsQry = $wpdb->get_var("SELECT reservation_id FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE SHA1(CONCAT(reservation_id, slot_id)) IN (".$listReservations.") LIMIT 1");
	...
	public function getCategoriesList($order_by) {
		...
		for($i=0;$i<count($categoriesQry);$i++) {
			...
			$translated_string = apply_filters( 'wpml_translate_single_string', stripslashes($categoriesQry[$i]->category_name), "wp-booking-calendar-categories", 'category_'.$categoriesQry[$i]->category_id );
			$arrayCategories[$categoriesQry[$i]->category_id]["category_name"] = $translated_string;
