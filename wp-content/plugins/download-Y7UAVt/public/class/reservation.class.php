...
class wp_booking_calendar_public_reservation {
	...
	public function setReservationByMD5($id) {
		...
		$reservationQry = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE SHA1(CONCAT(reservation_id, slot_id)) = %s",$id));
	...
	public function getReservationCalendarId() {
		global $wpdb;
		global $blog_id;
		return wp_booking_calendar_public_reservation::$reservationQry[0]->calendar_id;
	}
	
	
	public function insertReservation($bookingSettingObj,$fake = 0) {
		global $wpdb;
		global $blog_id;
		$blog_prefix=$blog_id."_";
		if($blog_id==1) {
		...
		for($i=0;$i<count($_POST["reservation_slot"]);$i++) {
			...
			if($bookingSettingObj->getSlotsUnlimited() != 1) {
                $rsCheck = $wpdb->get_results($wpdb->prepare("SELECT slot_av FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots WHERE slot_id=%d",$_POST["reservation_slot"][$i]));
                if(count($rsCheck)>0) {
                    ...
                    $avSeats = $rsSlot;
                    $ok = 0;
                    $rsRes = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE slot_id = %d AND reservation_cancelled=%d AND reservation_fake = %d",$_POST["reservation_slot"][$i],0,0));
                    if(count($rsRes)==0) {
                        $ok = 1;
                    } else {
                        $totSeats = 0;
			...
			if($ok == 1) {
				$wpdb->query($wpdb->prepare("INSERT INTO ".$wpdb->base_prefix.$blog_prefix."booking_reservation(slot_id,reservation_name,reservation_surname,reservation_email,reservation_phone,reservation_message,reservation_seats,reservation_field1,reservation_field2,reservation_field3,reservation_field4,calendar_id,post_id,wordpress_user_id,reservation_fake)
							 VALUES(%d,%s,%s,%s,%s,%s,%d,%s,%s,%s,%s,%d,%d,%d,%d)",$_POST["reservation_slot"][$i],$_POST["reservation_name"],$_POST["reservation_surname"],$_POST["reservation_email"],$_POST["reservation_phone"],$_POST["reservation_message"],$seats,$_POST["reservation_field1"],$_POST["reservation_field2"],$_POST["reservation_field3"],$_POST["reservation_field4"],$_POST["calendar_id"],$_POST["post_id"],$_POST["wordpress_user_id"],$fake));
		
				if($listReservations == "") {
					$listReservations.="".sha1($wpdb->insert_id.$_POST["reservation_slot"][$i])."";
				} else {
					$listReservations.=",".sha1($wpdb->insert_id.$_POST["reservation_slot"][$i])."";
				}
			}
	...
	public function confirmReservations($listIds) {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_reservation SET reservation_confirmed = %d, reservation_fake = %d WHERE SHA1(CONCAT(reservation_id, slot_id)) IN (".$listReservations.")",1,0));
	}
	...
	public function unfakeReservations($listIds) {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_reservation SET reservation_fake = %d WHERE SHA1(CONCAT(reservation_id, slot_id)) IN (".$listReservations.")",0));
	}
	...
	public function cancelReservations($listIds) {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_reservation SET reservation_cancelled = %d, reservation_confirmed = %d WHERE SHA1(CONCAT(reservation_id, slot_id)) IN (".$listReservations.")",1,0));
		$checkCalendar = "SELECT calendar_id FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE SHA1(CONCAT(reservation_id, slot_id)) IN (".$listReservations.")";
	...
	public function deleteReservations($listIds) {
		...
		$wpdb->query("DELETE FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE SHA1(CONCAT(reservation_id, slot_id)) IN (".$listReservations.")");
	}
	...
	public function checkReservationPaypalPaid($listIds) {
		...
		$arrayReservations = explode(",",$listIds);
		for($i=0;$i<count($arrayReservations);$i++) {
			$checkQry=$wpdb->query($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE SHA1(CONCAT(reservation_id, slot_id)) = %s AND reservation_confirmed = %d AND reservation_fake = 0",$arrayReservations[$i],1));
			if($checkQry>0) {
				$count++;
			}
		}
	...
	public function isPassed($listIds) {
		...
		$listReservations = "";
		for($i=0;$i<count($arrayReservations);$i++) {
			$reservationQry = $wpdb->get_results($wpdb->prepare("SELECT s.* FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation r INNER JOIN ".$wpdb->base_prefix.$blog_prefix."booking_slots s ON s.slot_id = r.slot_id WHERE SHA1(CONCAT(r.reservation_id, r.slot_id)) = %s",$arrayReservations[$i]));
	...
	public function isAdminConfirmed($listIds) {
		...
		$listReservations = "";
		for($i=0;$i<count($arrayReservations);$i++) {
			$reservationQry = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE SHA1(CONCAT(reservation_id, slot_id)) = %s",$arrayReservations[$i]));
    ...
    public function isCancelled($listIds) {
        ...
        for($i=0;$i<count($arrayReservations);$i++) {
            ...
            if($reservationQry[0]->reservation_cancelled == 1) {
                $result = true;
            }
    ...
    public function checkRecaptchaResponse($response,$privatekey) {
