...
class wp_booking_calendar_setting {
	...
	public function getRecaptchaVersion() {
		global $wpdb;
		global $blog_id;
		return wp_booking_calendar_setting::doSettingQuery('recaptcha_version');
	}
	...
	public function getReservationConfirmationModeOverride() {
		global $wpdb;
		global $blog_id;
		return wp_booking_calendar_setting::doSettingQuery('reservation_confirmation_mode_override');
	}
	
	public function getReservationAfterPayment() {
		global $wpdb;
		global $blog_id;
		return wp_booking_calendar_setting::doSettingQuery('reservation_after_payment');
	}
	...
	public function updateSettings() {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='reservation_confirmation_mode_override'",$_POST["reservation_confirmation_mode_override"]));
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='reservation_after_payment'",$_POST["reservation_after_payment"]));
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
		...
		update_option('wbc_name_from_reservation',$_POST["name_from_reservation"]);
        ...
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='recaptcha_version'",$_POST["recaptcha_version"]));
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
		...
		update_option('wbc_redirect_confirmation_path',$_POST["redirect_confirmation_path"]);
		
		update_option('wbc_redirect_booking_path',$_POST["redirect_booking_path"]);
		...
		update_option('wbc_redirect_cancel_path',$_POST["redirect_cancel_path"]);
		...
		update_option('wbc_terms_label',$_POST["terms_label"]);

		update_option('wbc_terms_link',$_POST["terms_link"]);
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
		...
		update_option('wbc_form_text',$_POST["form_text"]);
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
		...
		update_option('wbc_registration_text',$_POST["registration_text"]);
	}
    ...
    public function getDayNamesBg() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('day_names_bg'));
    }

    public function getDayBorder() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('day_border'));
    }
    ...
    public function getDayWhiteLine2DisabledBg() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('day_white_line2_disabled_bg'));
    }
    ...
    public function getDayWhiteLine2Bg() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('day_white_line2_bg'));
    }

    public function getDayWhiteLine2BgHover() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('day_white_line2_bg_hover'));
    }
    ...
    public function getDayRedLine2Bg() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('day_red_line2_bg'));
    }
    ...
    public function getMonthNavigationButtonColor() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('month_navigation_button_color'));
    }

    public function getMonthNavigationButtonColorHover() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('month_navigation_button_color_hover'));
    }
    ...
    public function getGoogleFontCssCode() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('google_font_css_code'));
    }

    public function getGoogleFontLinkCode() {
        global $wpdb;
        global $blog_id;
        return stripslashes(wp_booking_calendar_setting::doSettingQuery('google_font_link_code'));
    }
	...
	public function updateStyles() {
        ...
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='day_names_bg'",$_POST["day_names_bg"]));
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='day_border'",$_POST["day_border"]));
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
        ...
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='day_white_line2_disabled_bg'",$_POST["day_white_line2_disabled_bg"]));
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
        ...
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='day_white_line2_bg'",$_POST["day_white_line2_bg"]));
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='day_white_line2_bg_hover'",$_POST["day_white_line2_bg_hover"]));
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
        ...
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='day_red_line2_bg'",$_POST["day_red_line2_bg"]));
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
        ...
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='month_navigation_button_color'",$_POST["month_navigation_button_color"]));
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='month_navigation_button_color_hover'",$_POST["month_navigation_button_color_hover"]));
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
        ...
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='google_font_css_code'",$_POST["google_font_css_code"]));
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_config
					 SET config_value=%s
					 WHERE config_name='google_font_link_code'",$_POST["google_font_link_code"]));
	}
