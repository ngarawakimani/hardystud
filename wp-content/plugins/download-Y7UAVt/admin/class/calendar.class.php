...
class wp_booking_calendar_calendar {
	...
	public function delCalendars($listIds) {
		...
		if($default == 1) {
			$wpdb->query("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_calendars SET calendar_order = 0 WHERE calendar_order != 0 AND category_id = ".$category_id." ORDER BY calendar_order ASC LIMIT 1");
		}
	...
	public function addCalendar() {
		...
		do_action( 'wpml_register_string', $_POST["calendar_title"], "calendar_".$calendar_id, $package, "Calendar '".$_POST["calendar_title"]."' title", 'LINE' );
	...
	public function updateCalendar() {
		...
		do_action( 'wpml_register_string', $_POST["calendar_title"], "calendar_".$_POST["calendar_id"], $package, "Calendar '".$_POST["calendar_title"]."' title", 'LINE' );
	...
	public function duplicateCalendars($listIds) {
		...
		for($i=0;$i<count($calendarsQry);$i++) {
			...
			do_action( 'wpml_register_string', 'duplicate of '.$calendarsQry[$i]->calendar_title, "calendar_".$last_id, $package, "Calendar 'duplicate of ".$calendarsQry[$i]->calendar_title."' title", 'LINE' );
