...
if(wp_mail($to, $subject,$message, $headers )) {
	?>
    <script>
		window.parent.$wbc('#contact_modal').html('<?php echo esc_js(__( 'Message successfully sent!', 'wp-booking-calendar' )); ?><br><input type="button" onclick="hideModal()" value="close">');
	</script>
    <?php
} else {
	?>
    <script>
		window.parent.alert('<?php echo esc_js(__( 'An error has occurred. Retry', 'wp-booking-calendar' )); ?>');
	</script>
    <?php
}
