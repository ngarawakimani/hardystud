...
if($check >0) {
	...
	if(isset($_REQUEST["price"]) && $_REQUEST["price"]!='') {
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_slots SET slot_date= %s, slot_time_from = %s, slot_time_to = %s,slot_price=%d,slot_av=%d,slot_av_max=%d WHERE slot_id=%d",$_REQUEST["date"],$timeFromString.":00",$timeToString.":00",str_replace(",",".",$_REQUEST["price"]),$av,$avmax,$_REQUEST["item_id"]));
	} else {
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_slots SET slot_date= %s, slot_time_from = %s, slot_time_to = %s,slot_av=%d,slot_av_max=%d WHERE slot_id= %d",$_REQUEST["date"],$timeFromString.":00",$timeToString.":00",$av,$avmax,$_REQUEST["item_id"]));
	}
