	...
	function filterPagSlots(pag) {
		if($wbc("#search_date option:selected").val()==1) {
			...
			if(Trim($wbc('#first_date').val()) != '') {
				...
				$wbc.ajax({
				  url: '<?php echo BOOKING_ADMIN_URL;?>ajax/filterSlots.php?search_date=1&date_from='+$wbc('#first_date').val()+"&time_from="+$wbc('#time_from').val()+"&time_to="+$wbc('#time_to').val()+"&calendar_id=<?php echo $_GET["calendar_id"]; ?>&pag="+pag,
			...
			$wbc('#result_search').html('<img src="<?php echo BOOKING_ADMIN_URL;?>images/loading.gif">');
			$wbc.ajax({
			  url: '<?php echo BOOKING_ADMIN_URL;?>ajax/filterSlots.php?search_date=2&date_from='+$wbc('#first_date').val()+"&weekday="+$wbc('#slot_week_day').val()+"&date_to="+$wbc('#second_date').val()+"&time_from="+$wbc('#time_from').val()+"&time_to="+$wbc('#time_to').val()+"&calendar_id=<?php echo $_GET["calendar_id"]; ?>&pag="+pag,
			  success: function(data) {
...
/*pagination*/
if($totPages>1) {
	echo __( 'Pages', 'wp-booking-calendar' ).":&nbsp;";
	if($_GET["pag"]>1) {
		?>
		<a href="javascript:filterPagSlots(1);"><?php echo __( 'First', 'wp-booking-calendar' ); ?></a>
		
		<a href="javascript:filterPagSlots(<?php echo ($_GET["pag"]-1);?>);"><?php echo __( 'Prev', 'wp-booking-calendar' ); ?></a>
		<?php
	}
	...
	if($_GET["pag"]<$totPages) {
		?>
		<a href="javascript:filterPagSlots(<?php echo ($_GET["pag"]+1);?>);"><?php echo __( 'Next', 'wp-booking-calendar' ); ?></a>
		<a href="javascript:filterPagSlots(<?php echo $totPages; ?>);"><?php echo __( 'Last', 'wp-booking-calendar' ); ?></a>    
		<?php
	}
}
?>
<div id="table" class="booking_margin_t_20 booking_font_12">
	...
	<div class="booking_border_b_1 booking_border_solid booking_border_ccc booking_bg_f6f booking_height_30 booking_line_30">
        ...
        <div class="booking_float_left booking_width_10p"><?php echo __( 'Date', 'wp-booking-calendar' );?>&nbsp;<a href="javascript:orderby('date','<?php echo $_SESSION["orderbySlotsDate"]; ?>');"><img src="<?php echo BOOKING_ADMIN_URL;?>images/orderby_<?php echo $_SESSION["orderbySlotsDate"];?>.gif" border=0 /></a></div>
        
        <div class="booking_float_left booking_width_10p"><?php echo __( 'Time From', 'wp-booking-calendar' );?>&nbsp;<a href="javascript:orderby('time','<?php echo $_SESSION["orderbySlotsTime"]; ?>');"><img src="<?php echo BOOKING_ADMIN_URL;?>images/orderby_<?php echo $_SESSION["orderbySlotsTime"];?>.gif" border=0 /></a></div>
        
        <div class="booking_float_left booking_width_10p"><?php echo __( 'Time To', 'wp-booking-calendar' );?></div>
        
        <div class="booking_float_left booking_width_20p"><?php echo __( 'Special Text', 'wp-booking-calendar' );?></div>
        
        <div class="booking_float_left booking_width_7p"><?php echo __( 'Price', 'wp-booking-calendar' );?></div>
        
        <div class="booking_float_left booking_width_5p"><?php echo __( 'Seats', 'wp-booking-calendar' );?></div>
        
        <div class="booking_float_left booking_width_5p" style="height:30px;overflow:hidden"><?php echo __( 'Max bookable', 'wp-booking-calendar' );?></div>
        
        <div class="booking_float_left booking_width_10p"><?php echo __( 'Reserved', 'wp-booking-calendar' );?></div>
    ...
    <div id="row_<?php echo $slotId; ?>">
        ...
        <div class="booking_float_left booking_width_5p booking_height_50 <?php echo $class; ?>">
            <div class="booking_wh_inherit booking_table_cell booking_vertical_middle"><span id="modify_<?php echo $slotId; ?>"><a href="javascript:editItem(<?php echo $slotId; ?>,'<?php echo $slot["slot_reservation"]; ?>');"><?php echo __( 'Modify', 'wp-booking-calendar' );?></a></span></div>
        </div>
        
        
        <div class="booking_float_left booking_width_5p booking_height_50 <?php echo $class; ?>">
            <div class="booking_wh_inherit booking_table_cell booking_vertical_middle"><a href="javascript:delItem(<?php echo $slotId; ?>,'<?php echo $slot["slot_reservation"]; ?>');"><?php echo __( 'Delete', 'wp-booking-calendar' );?></a></div>
        </div>
        <div class="booking_cleardiv"></div>
    </div>
